...
class leerlingen:
    ...
    def get_naam(leerlingnummer):
        conn = sqlite3.connect("database.db")
        c = conn.cursor()
        c.execute("SELECT naam FROM leerlingen WHERE leerlingnummer = ?",
                  (leerlingnummer, ))
        naam = c.fetchone()[0]
        conn.close()
        return naam
    ...
    def set_mentor(naam, mentor):
        conn = sqlite3.connect("database.db")
        c = conn.cursor()
        c.execute("UPDATE leerlingen SET nieuwe_mentor = ? WHERE naam = ?",
                  (mentor, naam))
        conn.commit()
        conn.close()
    ...
    def get_mentor(naam):
        conn = sqlite3.connect("database.db")
        c = conn.cursor()
        c.execute("SELECT huidige_mentor FROM leerlingen WHERE naam = ?",
                  (naam, ))
        mentor = c.fetchone()[0]
        conn.close()
        return mentor
    ...
    def set_voorkeur(leerlingnummer, voorkeur_1, voorkeur_2, voorkeur_3):
        ...
        c = conn.cursor()
        c.execute(
            "UPDATE leerlingen SET voorkeur_1 = ?, voorkeur_2 = ?, voorkeur_3 = ? WHERE leerlingnummer = ?",
            (voorkeur_1, voorkeur_2, voorkeur_3, leerlingnummer),
        )
        conn.commit()
        conn.close()
        logging.info(
...
class mentoren:
    ...
    def set_voorkeur(mentor, voorkeur_1, voorkeur_2, voorkeur_3, voorkeur_4,
        ...
        c = conn.cursor()
        c.execute(
            "UPDATE mentoren SET voorkeur_1 = ?, voorkeur_2 = ?, voorkeur_3 = ?, voorkeur_4 = ?, voorkeur_5 = ? WHERE id = ?",
            (voorkeur_1, voorkeur_2, voorkeur_3, voorkeur_4, voorkeur_5,
             mentor),
        )
        conn.commit()
        conn.close()
    ...
    def get_allowed_students(mentor):
        ...
        c = conn.cursor()
        c.execute(
            "SELECT leerlingnummer, naam FROM leerlingen WHERE voorkeur_1 = ? OR voorkeur_2 = ? OR voorkeur_3 = ?",
